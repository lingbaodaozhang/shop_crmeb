...
class UserWithdrawalServices extends BaseServices
{
     ...
     * @return array
     */
    public function getWithdrawalList(array $where, string $field = '*', $is_page = true)
    {
        $whereData = [];
        if (isset($where['data'])) {
            $whereData['time'] = $where['data'];
		...
		$statusMap = ['待处理','已处理','已驳回'];
        foreach ($list as &$item) {
            $item['_finish_time'] = $item['finish_time'] ? date('Y-m-d H:i:s', $item['finish_time']) : '暂无';
            $item['_add_time'] = $item['add_time'] ? date('Y-m-d H:i:s', $item['add_time']) : '暂无';
            $item['status_type'] = $statusMap[$item['status']];
...
//        $data['sumWeixinPrice'] = $this->getRechargeSum($where, 'price');
     ...
     * @return array
     */
    public function user_withdrawal(array $where)
    {
        $data = [];
        $data['sumPrice'] = $this->getRechargeSum($where, 'price');
//        $data['sumRefundPrice'] = $this->getRechargeSum($where, 'refund_price');
...
//        $data['sumWeixinPrice'] = $this->getRechargeSum($where, 'price');
    ...
    public function confirm(int $id)
    {
        $rechargInfo = $this->getRecharge($id);
        if (!$rechargInfo) throw new AdminException(100026);
        if ($this->dao->update($id,['status' => 1,'finish_time' => time()])){
    ...
    {
        return 'tx' . date('YmdHis', time()) . substr(implode(NULL, array_map('ord', str_split(substr(uniqid(), 7, 13), 1))), 0, 8);
    }
